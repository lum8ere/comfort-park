...
const MINIO_MANAGER_KEY = "minio_manager"

func (sc *SmartContext) WithMinioManager(minioManager IMinioManager) ISmartContext {
	return sc.WithField(MINIO_MANAGER_KEY, minioManager)
}

func (sc *SmartContext) GetMinioManager() IMinioManager {
	result, ok := types.GetFieldTypedValue[IMinioManager](sc.dataFields, MINIO_MANAGER_KEY)
	if !ok {
		return nil
	}
	return result
